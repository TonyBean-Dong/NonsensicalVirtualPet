...
namespace F1yingBanana.SfizzUnity
{
    ...
    public sealed class SfizzMidiRenderer : MonoBehaviour
    {
        ...
        public async UniTask<AudioClip[]> RenderAsync(MidiFile midiFile, string[] sfzPaths, int sampleRate)
        {
            return await RenderAsync(midiFile, new SfizzArray(sfzPaths, sampleRate), sampleRate);
        }

        public async UniTask<AudioClip[]> RenderAsync(MidiFile midiFile, string sfzPath, int sampleRate)
        {
            return await RenderAsync(midiFile, new SfizzArray(sfzPath, sampleRate), sampleRate);
        }
        ...
        private async UniTask<AudioClip[]> RenderAsync(MidiFile midiFile, SfizzArray sfizzArray, int sampleRate)
        {
            AudioClip[] clips;
            TempoMap tempoMap = midiFile.GetTempoMap();
                ...
                midiFile.OriginalFormat == MidiFileFormat.MultiTrack)
            {
                // All channels should be rendered on the same buffer by add.
                clips = new AudioClip[1];
                clips[0] = await RenderEventsAsync(midiFile.GetTimedEvents(), tempoMap, sampleRate, sfizzArray);
                clips[0].name = "Track";
            }
            else
            {
                ...
                {
                    // Each track is rendered separately. Each track may still use multiple channels though.
                    clips[i] = await RenderEventsAsync(tracks[i].GetTimedEvents(), tempoMap, sampleRate, sfizzArray);
                    clips[i].name = $"Track {i}";
                }
            }
        ...
        private async UniTask<AudioClip> RenderEventsAsync(ICollection<TimedEvent> timedEventsCollection, TempoMap tempoMap,
            int sampleRate, SfizzArray sfizzArray)
        {
            if (timedEventsCollection.Count == 0)
            {
            ...
            await UniTask.RunOnThreadPool(() =>
            {
                while (timedEvents.Count > 0)
                {
                    ...
                    while (time > offset + Sfizz.MaxBlockSize)
                    {
                        ...
                        Array.Clear(interleavedBuffer, 0, interleavedBuffer.Length);
                        RenderBuffer(sfizzArray, buffer, interleavedBuffer, Sfizz.MaxBlockSize);
                        fullBuffer.AddRange(interleavedBuffer);
                        offset += Sfizz.MaxBlockSize;
                    }
                ...
                // Render the last buffer and finalize.
                int lastSize = totalDuration - offset;
                var lastBuffer = new float[lastSize * 2];
                RenderBuffer(sfizzArray, buffer, lastBuffer, lastSize);
                fullBuffer.AddRange(lastBuffer);
